...
function loadFromLocal() {
    ...
    subjects = JSON.parse(localStorage.getItem('subjects_' + key)) || [];
    totalXP = parseInt(localStorage.getItem('xp_' + key)) || 0;
    streak = parseInt(localStorage.getItem('streak_' + key)) || 0;
}

// Save to localStorage
function saveData() {
    const key = currentUser ? currentUser.id : 'guest';
    localStorage.setItem('subjects_' + key, JSON.stringify(subjects));
    localStorage.setItem('xp_' + key, totalXP);
    localStorage.setItem('streak_' + key, streak);
}
...
function checkStreak() {
    const key = currentUser ? currentUser.id : 'guest';
    const lastStudy = localStorage.getItem('lastStudy_' + key);
    ...
    localStorage.setItem('lastStudy_' + key, today);
    saveData();
    updateStats();
}
